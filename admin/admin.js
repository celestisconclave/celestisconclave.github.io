...
function updatePendingSummary() {
  ...
  if (!el) return;
  if (pendingChanges) {
    el.textContent = '⚠ You have unsaved local changes. Enter your GitHub PAT above and click "Push Changes" to publish.';
...
function renderProjectsList() {
  ...
  list.innerHTML = localData.projects.map(p => `
...
function renderArticlesList() {
  ...
  list.innerHTML = localData.articles.map(a => `
...
function renderResourcesList() {
  ...
  list.innerHTML = localData.resources.map(r => `
...
function renderTeamList() {
  ...
  if (!list) return;
  list.innerHTML = localData.team.map(m => `
...
window.saveProject = function(id) {
  ...
  if (!title || !shortDesc) { showToast('Title and short description are required.', 'error'); return; }

  const tags = [...document.getElementById('proj-tags-wrap').querySelectorAll('.tag-removable')].map(el => el.dataset.tag);
...
window.saveArticle = function(id) {
  ...
  if (!title || !author || !date || !preview) { showToast('Please fill required fields.', 'error'); return; }

  const tags = [...document.getElementById('art-tags-wrap').querySelectorAll('.tag-removable')].map(el => el.dataset.tag);
  const imgFile = document.getElementById('art-img-file').files[0];
  const mdFile = document.getElementById('art-md-file').files[0];

  const fname = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');

  if (!window._pendingFiles) window._pendingFiles = {};

  let imgPath = null, mdPath = null;
  if (imgFile) {
  ...
  if (id) {
    ...
    localData.articles.push({
      id: newId, title, author, date, preview, tags,
...
window.saveResource = function(id) {
  ...
  if (!title || !link || !description) { showToast('Please fill required fields.', 'error'); return; }

  const tags = [...document.getElementById('res-tags-wrap').querySelectorAll('.tag-removable')].map(el => el.dataset.tag);
...
window.saveTeamMember = function(id) {
  ...
  if (!name || !role || !bio) { showToast('Name, role and bio are required.', 'error'); return; }
  ...
  if (id) {
    ...
    if (mem) {
      mem.name = name; mem.role = role; mem.bio = bio; mem.socials = socials;
      if (photoPath) mem.photo = photoPath;
    }
  } else {
    const newId = Math.max(0, ...localData.team.map(m => m.id)) + 1;
    localData.team.push({ id: newId, name, role, bio, socials, photo: photoPath || 'assets/images/team/placeholder.jpg' });
  }
...
window.confirmDelete = function(type, id) {
  const titles = { project: 'Delete Project', article: 'Delete Article', resource: 'Delete Resource', team: 'Remove Team Member' };
  ...
  document.getElementById('confirm-ok').addEventListener('click', () => {
    ...
    markChanged();
    renderProjectsList(); renderArticlesList(); renderResourcesList(); renderTeamList();
    renderOverview();
    cleanup();
