...
function initSolarSystem() {
  ...
  function drawOrbit(p, highlighted) {
    ...
    if (highlighted) {
      ctx.strokeStyle = 'rgba(212,168,67,0.55)'; // gold
      ctx.lineWidth   = 1.5;
    } else {
  ...
  function drawPlanet(p, angle, highlighted) {
    ...
    const glowColor = highlighted ? 'rgba(212,168,67,' : `rgba(${p._rgb},`;
    ...
    ctx.fillStyle    = highlighted ? '#f0c96e' : p.color; // gold tint when lit
    ctx.shadowColor  = highlighted ? '#d4a843' : p.color;
    ...
    if (highlighted) {
      ...
      ctx.fillStyle = 'rgba(240,201,110,0.9)';
