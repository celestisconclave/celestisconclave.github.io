...
function renderProjectFilters() {
  ...
  container.innerHTML = tags.map(t =>
    `<button class="tag-pill ${activeProjectTags.has(t) ? 'active' : ''}" data-tag="${t}">${t}</button>`
  ).join('');
  container.querySelectorAll('.tag-pill').forEach(btn => {
    btn.addEventListener('click', () => {
      const tag = btn.dataset.tag;
      if (activeProjectTags.has(tag)) { activeProjectTags.delete(tag); btn.classList.remove('active'); }
      else { activeProjectTags.add(tag); btn.classList.add('active'); }
      renderProjectsGrid();
    });
  });
}
...
function openProjectModal(id) {
  ...
  content.innerHTML = `
    ...
    <div class="modal-body">
      ...
      ${p.link ? `<a href="${p.link}" target="_blank" rel="noopener" class="btn btn-primary mt-md">
        View Project →
      </a>` : ''}
    </div>
  `;
